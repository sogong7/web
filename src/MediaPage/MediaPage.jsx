...
export default class MediaPage extends Component {
    ...
    async componentDidMount(){
        const api_detail = await this._detailApi()
        this.setState({
            medias: api_detail[0]
        })
            console.log('media',this.state.medias)
    }
    _detailApi = () => {
    ...
    render() {
        ...
        return (<div>
            <Header />
            {media&&<div className='media_page_wrap'>
                <div className='media_page clear_float'>
                    <div className="media_info clear_float" style={{width:"1080px", margin: "auto"}}>
                        <div className="media_poster" style={{backgroundImage: `URL(${media.medium_cover_image})`, backgroundSize: 'cover' }}></div>
                        <h1><span className="title"> </span>{media.title}</h1>
                        <p><span className="title">출시년도 : </span>{media.year}</p>
                        <p><span className="title">감독 : </span>
                            {
                            media&&media.genres.map((movieStar, index) => (
                                index == media.genres.length-1 ? movieStar : movieStar +', '
                            ))
                            }
                        </p>    
                        <p><span className="title">상영시간 : </span>
                            {media.runtime}분
                        </p>
                        <p><span className="title">별점 : </span>{media.rating} / {10}</p>
                    </div>
                    <div className="media_info" style={{width:"1080px", margin: "auto"}}>
                        <p className="title">줄거리</p>
                        <p>{media.description_full}</p>
                    </div>
                </div>
            </div>
            }
...
// media&&media.movieStar.map((movieStar, index) => (
